...
2. **Given** any in-app request has been waiting longer than ~3 seconds, **When** the wait continues, **Then** the user sees an explicit "el servidor está despertando…" state (in español neutro) instead of an indefinite spinner or an error.
...
- **FR-002**: Device retention MUST operate on an explicit allow-list only. Initially: competition calendar events, race events and standings, revision-reason catalogs, competition lists, and training-session lists. Everything not allow-listed MUST NOT be stored — explicitly including anthropometry/PHV records, medical data, dates of birth, parent-specific data, newsletters, per-athlete AI-generated content, and credentials/tokens beyond what the app already stores today. New content types default to *not stored*.
...
- **FR-008**: When any request has been waiting longer than ~3 seconds, the app MUST display an explicit "el servidor está despertando…" state (español neutro, Colombia) instead of a generic spinner, and MUST clear it automatically when the response arrives.
...
- The initial allow-list is curated with a privacy review (data-privacy-guard audit per the constitution) and may only grow through the same review.
