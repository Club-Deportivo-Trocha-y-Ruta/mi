...
- [X] T001 Review and finalize the canonical glossary in `specs/001-translate-claude-files-english/data-model.md` (add any recurring Spanish term missing from the corpus; confirm "do not translate" list for stack/proper nouns)
- [X] T002 [P] Build the preserved-token allow-list (proper nouns, enum values `Pre-PHV`/`Circa-PHV`/`Post-PHV`, template names, seed emails, stack names) as a reference note in `specs/001-translate-claude-files-english/data-model.md`
- [X] T003 [P] Confirm verification commands from `specs/001-translate-claude-files-english/quickstart.md` run in this environment (`python3` + `pyyaml` available for INV-2; `git diff`, `grep` for INV-5/INV-7)
...
- [X] T004 Create verification helper script `specs/001-translate-claude-files-english/verify.sh` implementing INV-1 (token diff), INV-3 (heading/table/code-block parity), and INV-4 (residual-Spanish prose scan) per `contracts/translation-invariants.md`
- [X] T005 [P] Add INV-2 frontmatter check (parse all `.claude/agents/*.md`, assert only `description` changed) to `specs/001-translate-claude-files-english/verify.sh`
- [X] T006 [P] Add INV-6 (link resolution) and INV-7 (out-of-scope guard via `git diff --name-only` allow-list) to `specs/001-translate-claude-files-english/verify.sh`
- [X] T007 Capture a baseline token/heading snapshot of all in-scope source files (pre-translation) so INV-1/INV-3 can diff source vs. target
...
- [X] T008 [US1] Translate `CLAUDE.md` prose to English (Identity, Reference docs, Stack, Architecture, Data model, Production, Deploy, all Implementation-status sections, Dev credentials note, Technical notes, Dev commands, Calendar labels, Non-negotiable principles, Age-group differentiation, Session format labels, Privacy, Context-compaction) applying the glossary; preserve all code blocks, paths, env vars, enum values, dates, URLs, and the Copa Valle calendar data per `data-model.md`
- [X] T009 [US1] Rewrite the `CLAUDE.md` "## Idioma" section as "## Language": instruct the AI dev-assistant to operate and respond in English, and explicitly affirm that product end-user copy (emails/PDF/UI in code) stays Spanish (Colombia) — implements FR-005 (must mirror the amended constitution Principle III from T031)
- [X] T010 [US1] Translate the mandatory "Formato de sesiones de entrenamiento" template: section labels to English (`CALENTAMIENTO`→`WARM-UP`, `PARTE PRINCIPAL`→`MAIN SET`, `VUELTA A LA CALMA`→`COOL-DOWN`, `Notas`→`Notes`) while preserving 🚴/📅/⏱/💡 markers (FR-007)
- [X] T011 [US1] Run `verify.sh` against `CLAUDE.md` (INV-1, INV-3, INV-4, INV-6); fix any token/structure/link regressions
- [X] T011a [US1] Triage INV-6 link findings on `CLAUDE.md`: the reference to `docs/03-fase1/workflow.md` is a PRE-EXISTING broken link (folder absent). Decide per ref — correct the path, or remove/annotate the stale reference — and record the decision. Do not let a pre-existing break silently fail the gate
...
- [X] T013 [P] [US2] Translate data/privacy agents: `.claude/agents/data-platform-lead.md`, `data-analyst.md`, `results-analyst.md`, `data-privacy-guard.md`, `analytics-reporter.md` (preserve `services/race/*`, CLI command names, enum values)
- [X] T014 [P] [US2] Translate engineering agents: `.claude/agents/engineering-lead.md`, `fastapi-architect.md`, `react-ui-engineer.md`, `devops-engineer.md`, `qa-engineer.md`, `database-architect.md`, `integration-engineer.md` (preserve stack names, file paths, identifiers)
- [X] T015 [P] [US2] Translate family/comms agents: `.claude/agents/family-relations-lead.md`, `parent-communicator.md`, `community-content-creator.md`, `event-coordinator.md` (preserve Resend template names, privacy notes; keep Spanish product copy examples flagged as deliberate)
- [X] T016 [P] [US2] Translate product agents: `.claude/agents/product-manager.md`, `release-manager.md`, `technical-writer.md`, `ux-researcher.md` (preserve doc paths, convention references)
- [X] T017 [US2] Run `verify.sh` INV-2 (frontmatter integrity, only `description` changed) across all 28 agent files; fix regressions
- [X] T018 [US2] Run `verify.sh` INV-1/INV-3/INV-4/INV-6 across all 28 agent files; confirm 0 broken slug/path references and 0 residual Spanish prose
...
- [X] T019 [P] [US3] Translate root docs: `docs/01-marco-teorico.md` and `docs/README.md`
- [X] T020 [P] [US3] Translate `docs/02-scaffolding/` markdown files
- [X] T021 [P] [US3] Translate `docs/04-percentiles/` markdown files
- [X] T022 [P] [US3] Translate `docs/05-design-system/` markdown files
- [X] T023 [P] [US3] Translate `docs/06-parents/` markdown files
- [X] T024 [P] [US3] Translate `docs/07-notifications/` markdown files
- [X] T025 [P] [US3] Translate `docs/08-onboarding/` markdown files
- [X] T026 [P] [US3] Translate `docs/09-training-planning/` markdown files (preserve `snapshots/*.yml` fixtures untouched)
- [X] T027 [P] [US3] Translate `docs/10-race-results/` markdown files (preserve `snapshots/*.pdf` and any fixtures untouched)
- [X] T028 [P] [US3] Translate `docs/11-informe-tecnico-mensual/` markdown files
- [X] T029 [P] [US3] Translate `docs/12-competitions-unification/` markdown files
- [X] T030 [US3] Run `verify.sh` INV-1/INV-3/INV-4/INV-6/INV-7 across `docs/**`; confirm binary/fixture assets are byte-identical and all links resolve
...
- [X] T031 Via `/speckit-constitution`, amend `.specify/memory/constitution.md` Principle III ("User Experience Consistency") to codify: AI dev-assistant working language = English; product end-user copy = español neutro (Colombia). Do not change other principles
- [X] T032 (handled by `/speckit-constitution`) Verify the Sync Impact Report comment is updated and the version is bumped (MINOR per its versioning policy) with Ratified/Last-Amended dates
- [X] T033 Run INV-9 (language-policy coherence): cross-read `CLAUDE.md` "Language" + amended constitution Principle III; confirm single coherent policy + version/report delta
...
- [X] T034 Run full `verify.sh` across the entire in-scope set (INV-1…INV-7) and confirm all green
- [X] T035 INV-5 idempotency: run a second translation pass over all translated files; assert `git diff --quiet` (no further changes)
- [X] T036 [P] INV-8 privacy sweep: confirm no minor PII introduced in any file or commit message; placeholders remain placeholders
- [X] T037 [P] Execute `specs/001-translate-claude-files-english/quickstart.md` end-to-end as a final validation walkthrough
- [X] T038 Update `specs/001-translate-claude-files-english/checklists/requirements.md` if any item state changed; record final pass counts
