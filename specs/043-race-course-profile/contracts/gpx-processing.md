...
Constants (module level, imported by tests and echoed in `LapDetection`): `MIN_LAP_M = 300`, `MAX_LAP_M = 15_000`, `CLOSURE_RADIUS_M = 30`, `LAP_LENGTH_TOLERANCE = 0.10`, `SMOOTH_WINDOW_M = 30`, `GAIN_HYSTERESIS_M = 3`, `SIMPLIFY_EPSILON_M = 3.0`, `MAX_POINTS = 800`, `MIN_POINTS = 20`, `MAX_DISTANCE_ERROR = 0.01`, `MAX_RAW_POINTS = 20_000` (T068 mitigation, added 2026-09-16).
...
4. **Sanity**: `len(points) == 0` → `no_track_points`; `len(points) > MAX_RAW_POINTS` (20,000 — a real 1 Hz lap recording, even at `MAX_LAP_M`, is on the order of 2,000–4,000 points) → `too_many_points`, cutting before the cumulative-distance/lap-extraction/RDP cost that a pathological file would otherwise pay (T068, `plan.md` §Complexity Tracking: a synthetic 60,000-point GPX measured p95 ≈ 3.18 s without this check); any point with lat/lon outside range or NaN → `no_position`.
