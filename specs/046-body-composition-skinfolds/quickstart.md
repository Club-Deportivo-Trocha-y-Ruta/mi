...
Expected: scenario set A–G of `contracts/body-composition-reading.md` §5 passes; parent `PUT` 403, parent `GET body-composition` 403, interval 409 with `next_allowed_date`, parent growth-summary block has exactly five keys; no numeric key in the AI leaf.
...
5. `GET …/growth-summary` as the linked parent → `body_composition` has only `has_data, latest_set_date, band, family_label, family_sentence`.
