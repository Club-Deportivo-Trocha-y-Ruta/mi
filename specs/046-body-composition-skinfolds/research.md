...
- **Decision**: parents receive `body_composition` in `growth-summary` as `{has_data, latest_set_date, family_band, family_label, family_sentence}` only, where `family_band` is a projection capped at ámbar (coach rojo → ámbar; reference-only ámbar → verde; `contracts/body-composition-reading.md` §3c, spec clarifications Q1/Q4) — the coach `band` never leaves coach payloads; the monthly newsletter carries the same projection as a deterministic fixed-copy block only in the month of a counted set, never through the newsletter AI (spec FR-036, clarifications Q2/Q3; the newsletter pipeline has no golden eval, so its prompt must not change); `AnthropometryOut.skinfolds` is set to `None` for parents in the list endpoint (same block that already strips `notes`/`morphology`); `GET …/body-composition` returns 403 for parents; the existing parent-downloadable `anthropometry_report.html` is **not** extended with skinfolds at all (simplest guarantee of FR-026); the coach-only PDFs are the field guide and the referral note.
