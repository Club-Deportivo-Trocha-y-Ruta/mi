...
> **Note from gate G4 (2026-09-21)**: before the real load, cache the corrected parsed rows per staged import (key `(sha256, corrections revision)`) so `/rebuild` and the commit gate stop re-downloading and re-parsing every staged PDF — see `plan.md` Complexity Tracking.

- [ ] T061 [US5] `POST /{id}/commit-pending` (behind the same identity-review gate as `/commit`, FR-018 — reuse its rebuild-then-check pattern: run `identity_review.rebuild` with `load_identity_rows` first, then 409 `identity_review_pending` when pending > 0) and the `pending_categories` bookkeeping in `backend/app/routers/race_imports.py`; `pending_categories_count` in the import list schema [agent: fastapi-architect · sonnet]
