...
- **FR-044**: Every way of loading results by uploading a file (PDF, CSV or any other format) MUST be removed from the web app and the API for every role; no endpoint may accept a results file, and this MUST be covered by a test.
...
- **FR-047**: The skill's load script MUST write directly to the target database. It MUST target the local database by default and MUST write to production only when the operator asks for it explicitly (a dedicated flag plus a confirmation naming the target); production credentials are read from a git-ignored environment file and never printed. Results MUST NOT be loaded through Alembic data migrations or any other file committed to the repository, and the extracted data MUST stay in a git-ignored folder on the operator's computer.
...
- **SC-014**: 0 endpoints or screens accept a results file upload, for any role.
