...
The coach hands the platform an official results file of a past válida and gets back a preview that accounts for every printed row. Today that is not true for the historical files: when a rider's city or club is long enough to wrap onto two or three printed lines, the row is dropped without any warning (measured on the first válida of 2025: 5 of 18 rows of one children's category, 28 %, including the winner), and whole categories whose printed header is not recognised disappear from the preview (5 categories in that same file, 9 in the first válida of 2024, among them the complete elite men's field). After this story, wrapped rows are reassembled, every category header is either recognised or shown to the coach as unrecognised, and each category is checked for completeness: the positions of its classified finishers must form the full sequence 1…N. A category that fails the check is marked inconsistent, shows which positions are missing or duplicated, and cannot be committed until the coach corrects it by hand. The other categories of the same válida are not held back.
...
**Independent Test**: With three synthetic results files that reproduce the historical layout (long wrapped city and club values, historical category headers, one category with a deliberately removed row), run the preview and assert that every wrapped row appears once with its position, time and points; that no category is dropped without being listed as unrecognised; that the category with the removed row is marked inconsistent naming the missing position; and that committing it is refused while the remaining categories commit normally.

**Acceptance Scenarios**:

1. **Given** a results file where a rider's city or club wraps onto several printed lines, **When** the file is previewed, **Then** that rider appears as a single complete row with position, name, club, time and points.
...
6. **Given** a gap that really exists in the official file, **When** the coach acknowledges it with a mandatory written reason, **Then** the category can be committed and the acknowledgement is recorded in the audit trail with author and time.
...
With reading, categories, the privacy guarantee and the identity review in place, the coach loads the seven válidas of 2024 and the eight of 2025, each from its official per-válida results file. The load uses the same preview → dry-run → commit path, the same duplicate protection and the same audit trail as the 2026 imports; it is not a separate route with different rules. The 2024 and 2025 seasons appear as seasons of the same cup, each válida with its date and venue, and their standings are calculated from the loaded results. The cumulative standings files published by the organiser are not loaded.
...
1. **Given** the official results file of a historical válida, **When** the coach loads it, **Then** the válida exists in its own season with date and venue and all its consistent categories.
2. **Given** a file that was already loaded, **When** it is loaded again, **Then** no duplicate válida, competitor or result is created and the coach is told it was already loaded.
...
5. **Given** a file whose season, válida number or venue cannot be read from its header, **When** it is previewed, **Then** the coach is asked to supply them and nothing is assumed.
...
- The official file itself contains a position gap (for example after a disqualification): the coach acknowledges it with a written reason (User Story 1, scenario 6).
...
- A results file is scanned rather than text: it is rejected with a clear message; reading images is out of scope.
...
- **FR-001**: The platform MUST reassemble a result row that is printed across several physical lines into one complete row, and MUST NOT discard any printed row without listing it to the coach.
- **FR-002**: The platform MUST list every category header it does not recognise, with the printed header and its row count, and MUST NOT drop a category silently.
...
- **FR-004**: The coach MUST be able to correct an inconsistent category by hand in the preview; the check MUST run again after each correction and the block MUST lift when the sequence is complete.
- **FR-005**: The coach MUST be able to acknowledge a gap that exists in the official file by entering a mandatory written reason; the acknowledgement MUST be audited with author and time and MUST unblock only that category.
- **FR-006**: An inconsistent category MUST NOT block the commit of the consistent categories of the same válida.
- **FR-007**: Files of the 2026 season that are read correctly today MUST produce an identical preview after this feature.
...
- **FR-023**: The 2024 and 2025 seasons MUST be loaded through the same preview → dry-run → commit path, duplicate protection and audit trail as the current season's imports; no parallel path with different rules may exist.
- **FR-024**: Only the per-válida results file is ingested; the organiser's cumulative standings files MUST NOT be loaded.
- **FR-025**: Each historical season MUST appear as a season of the same cup with its válidas, dates and venues; when the header of a file does not yield season, válida number, date or venue, the coach MUST be asked and nothing is assumed.
...
- **FR-030**: For each result of a club athlete the platform MUST provide the field size, defined as the number of full-distance classified finishers of the same category and válida.
- **FR-031**: The platform MUST provide the gap to the median as (athlete's time − median time of full-distance finishers of the same category and válida) ÷ that median × 100, to one decimal, only when the field size is at least five and the athlete finished the full distance with a time; otherwise "sin dato".
...
- **SC-001**: On the reference historical files, 100 % of printed result rows appear in the preview (row count per category equals the printed count) and 0 categories disappear without being listed; the 28 % loss measured on the 2025 opening válida drops to 0 %.
- **SC-002**: Across the fifteen historical válidas, 0 categories are committed with an incomplete position sequence unless an audited acknowledgement with a written reason exists.
...
- **SC-012**: Gap to the median and percentile read "sin dato" in 100 % of válidas with fewer than five full-distance finishers, and average speed reads "sin dato" in 100 % of válidas without a course profile; no estimated figure is ever shown.
...
- **Sources**: the organiser publishes each válida's results file through links on its public results pages for 2024 and 2025. The coach (or an operator on the coach's behalf) obtains the fifteen files and provides them to the platform; automatic crawling of the organiser's site is out of scope. A sample of three of those files was inspected on 2026-09-18: all are text-based documents of the same visual family as the 2026 files.
- **Evidence behind the reading requirements**: on that sample, one children's category of the 2025 opening válida lost 5 of 18 rows including the winner; 5 categories of that file and 9 of the 2024 opening válida (including the full elite men's field) were discarded for unrecognised headers. These numbers are the baseline for SC-001.
...
- **Average speed in historical válidas**: most historical válidas will have no course profile, so speed will read "sin dato" there; the gap to the median is therefore the series every athlete will have. A course profile can be added to a historical válida later through the existing course-profile flow.
- **Completeness rule and ties**: official results are assumed not to contain ties; a duplicated position is treated as an inconsistency to be corrected or acknowledged.
...
- **Stored source files**: the import path keeps the provided file as it does today for 2026; how long those files are kept is the retention question deferred by FR-043.
