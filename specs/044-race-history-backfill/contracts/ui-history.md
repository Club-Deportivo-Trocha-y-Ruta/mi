...
- Chart (Recharts `ComposedChart`, time-scale x): a single metric, **Brecha a la mediana** — no toggle (owner decision 2026-09-22: average speed removed from the cross-season history, "no es un dato relevante" here; it still lives per-válida on the Circuito tab and `EvolutionChart`/`EvolutionTable`, feature 043 — untouched). Gap axis inverted so up = faster; zero line labelled "Mediana de su categoría". `ReferenceLine` per `category_changed` point, label "Infantil B → Prejuvenil A". Dashed connector across missing válidas/seasons; DNF/DSQ/DNS drawn as a hollow marker on the baseline with its status in the tooltip and no value. Tooltip always shows "Parrilla: 23". One neutral series colour; status colours untouched. Chart has a text alternative (the table below) and `aria-describedby` to the caveats. Ticks span the full x-axis domain even across season boundaries.
- Table grouped by season → category_id (not the frozen printed label — a rename within a season must not split the group; the heading uses the most recent printed label, or "A / B" when several apply, and each row still shows its own printed label where it differs): fecha, válida, puesto, percentil, parrilla, brecha. No row or line joins two groups. "sin dato" wherever the API sends `null`.
