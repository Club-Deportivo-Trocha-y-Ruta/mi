...
- **Delivery mapping — this spec is the program index**: each story is specified in full as its own executable feature spec (created 2026-07-11): Story 1 → `specs/028-frontend-design-foundation` (phases 0–1) · Story 2 → `specs/030-coach-navigation-redesign` (phase 3) · Story 3 → `specs/029-coach-surface-subtraction` (phase 2) · Story 4 → `specs/031-coach-home-mission-control` (phase 4) · Story 5 → `specs/032-session-content-unification` (phase 5) · Stories 6–7 → `specs/033-visual-coherence-polish` (visual half of phase 1 + phase 6). Recommended implementation order: 028 → 029 → 030 → 031 → 032 → 033. Plan and implement the child specs individually (`/speckit-plan` per feature); this program spec is not planned directly — it holds the program-level scope, decisions D1–D6, and success criteria the children inherit.
