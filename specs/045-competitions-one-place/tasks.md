...
- [X] T021 [US3] Write failing-first tests in `backend/tests/routers/test_race_imports_identity_gate.py`. With the whole-queue gate these fail today:
...
- [X] T022 [US3] Add `pending_candidates_for_import(db, import_record_keys) -> list[RaceIdentityCandidate]` in `backend/app/services/race/identity_review.py`. Follow the `remove_out_of_scope` pattern (~l.984-1027) and match `left_record["key"]` or `right_record["key"]` (research R-08). Add a unit test in `backend/tests/services/race/test_identity_review.py`.
- [X] T023 [US3] Replace `_identity_pending_count` at commit (~l.1223) and commit-pending (~l.1506) in `backend/app/routers/race_imports.py`:
  ...
  - T021 goes green.
- [X] T024 [US3] Write the Alembic migration `backend/alembic/versions/<rev>_race_import_discarded.py`:
...
- [X] T027 [P] [US5] Extend `CoachSummaryOut` in `backend/app/schemas/dashboard.py` with `identity_decisions_pending: int | None`, `imports_in_progress: int | None` and `analyses_awaiting_approval: int | None`. Compute them in `backend/app/services/dashboard_summary.py`, following `compute_insights_stale` (~l.122-165):
...
- [X] T028 [US5] Add `GET /api/race-analysis/pending-analyses?season=&state=awaiting_approval|stale` in `backend/app/routers/race_analysis.py` (contracts/api.md). Its items must match the T027 counts exactly. Tests: coach 200 with an equality assertion against the counts, parent 403, unknown state 422. Never log `athlete_ref`.
- [X] T029 [US5] Add `POST /api/race-analysis/runs/{run_id}/dismiss-stale` in `backend/app/routers/race_analysis.py`. It wraps `backend/app/services/race/run_staleness.py::mark_run_fresh` and writes an audit event (`action=dismiss_stale`). Tests: 200 (the run leaves the stale list), 409 when not stale, parent 403.
...
- [X] T033 [P] [US1] Write failing-first tests in `frontend/src/components/athletes/races/__tests__/CarrerasTab.test.tsx`:
...
- [X] T034 [P] [US1] Write failing-first tests in `frontend/src/routes/athletes/AthleteDetailPage.test.tsx`:
...
- [X] T035 [US1] Extend `frontend/src/hooks/race/useAthleteRaceHistory.ts` to accept `seriesKind: "cup" | "championship" | "all"` (default `"cup"` for existing callers).
- [X] T036 [US1] Extend `frontend/src/components/race/history/HistoryChart.tsx` with a `metric` prop:
...
- [X] T037 [US1] Create `frontend/src/components/athletes/races/ProgressionView.tsx`:
...
- [X] T038 [US1] Create `frontend/src/components/athletes/races/AnalysisView.tsx`:
...
- [X] T039 [US1] Create `frontend/src/components/athletes/races/CompareView.tsx` (coach only) with `DistributionChart` + `ComparatorPanel`.
...
- [X] T040 [US1] Create `frontend/src/components/athletes/races/CarrerasTab.tsx`:
  ...
  T033 goes green.
- [X] T041 [US1] In `frontend/src/routes/athletes/AthleteDetailPage.tsx`, replace the `ai_analysis` and `races` tabs with one lazy «Carreras» tab (key `races`) and add the alias parsing for `ai_analysis`. T034 goes green.
- [X] T042 [US1] Retire the unused pieces:
...
- [X] T043 [US1] Point `frontend/src/routes/RaceInsightEmailRedirect.tsx` to `?tab=races&view=analisis&insight=<iid>` for both roles, and update its test.
...
- [X] T044 [P] [US4] Write failing-first tests in `frontend/src/routes/parents/MyAthleteDetailPage.test.tsx`:
  ...
  - jest-axe reports zero violations.
- [X] T045 [US4] In `frontend/src/routes/parents/MyAthleteDetailPage.tsx`, replace the `ai-analysis` and `races` tabs with a lazy «Carreras» (`CarrerasTab` with `audience="family"`). Parse the `ai-analysis` alias and keep the path from the family home at two taps or fewer (FR-018). T044 goes green.
- [X] T046 [US1] GATE G3. Run:
  - `npm run typecheck`;
...
- [X] T047 [P] [US6] In `frontend/src/lib/navigation.ts`, make the area «Competencias» with the items «Competencias» (`/competitions`), «Temporada» (`/competitions/season/:currentYear`) and «Cargas e identidades» (`/competitions/imports`), and remove «Válidas». Test the menu labels.
- [X] T048 [US6] Add the routes `/competitions/season/:year` and `/competitions/imports` to `frontend/src/App.tsx`, plus the redirects from `contracts/ui-routes.md`:
...
- [X] T049 [P] [US6] Create `frontend/src/components/competitions/tabs/CircuitAndConditionsTab.tsx`, composing `ConditionsTab` and `race/course/CourseTab`. In `frontend/src/routes/competitions/CompetitionDetailPage.tsx`:
...
- [X] T051 [P] [US6] Replace «Crítico LLM dice» with «Revisión automática» in `frontend/src/components/ai/HITLApprovalCard.tsx` (~l.207), and add `min-h-12` to «Editar» (~l.263) and «Rechazar» (~l.273). Test the labels and target classes.
- [X] T052 [P] [US6] Add `min-h-12` to the `CompetitionsListPage.tsx` header buttons (~l.198-225), and spell out the priority codes through one label map (grep for the raw «CD» rendering). Add tests.

### US3 — «Cargas e identidades» and resumable imports, frontend part (P1)

- [X] T053 [US3] Extract the page bodies of `frontend/src/routes/competitions/history/HistoricalLoadPage.tsx`, `history/IdentityReviewPage.tsx` and `UnlinkedCompetitorsPage.tsx` into section components under `frontend/src/components/competitions/imports/` (`LoadsSection.tsx`, `IdentitySection.tsx`, `UnlinkedSection.tsx`). Then create `frontend/src/routes/competitions/CompetitionImportsPage.tsx` with `?seccion=cargas|identidades|sin-enlazar`. Add tests and jest-axe.
- [X] T054 [US3] Add `getRaceImport(id)` and `discardRaceImport(id)` to `frontend/src/api/raceImports.ts`, with hooks. In `ImportWizard.tsx`:
...
- [X] T055 [P] [US3] In `frontend/src/hooks/layout/useNavBadges.ts`, show the «Cargas e identidades» badge as `identity_decisions_pending + imports_in_progress`, and extend the coach-summary types in `frontend/src/hooks/dashboard/useCoachSummary.ts` and `frontend/src/api/dashboard.ts`. Add tests.

### US5 — «Pendientes» and «Temporada», frontend part (P2)

- [X] T056 [US5] In `frontend/src/components/dashboard/PendingInbox.tsx`:
  ...
  Add tests.
- [X] T057 [US5] In `frontend/src/routes/competitions/SeasonInsightsPage.tsx` («Temporada»):
...
- [X] T059 [US6] GATE G4. Run the T046 commands and the T020 backend commands. Also check that the user's untracked scripts in `backend/scripts/` still exist.

---

## Phase 7: W5 — family copy and the approval warning · US4

- [X] T060 [P] [US4] In `backend/app/services/training/stage_log_builder.py` (~l.180-190), replace the waypoint sublabel «±N% al P1» with «Brecha vs. mediana», computed live from the engine at render time. Test in the stage-log builder tests (grep `stage_log_builder` under `backend/tests/`) that no «P1» or winner text remains.
- [X] T061 [P] [US4] In `backend/app/services/training/newsletter_builder.py` (~l.1460-1554), replace the `gap_pcts` chart series with the median gap from the engine for family renders, and add a test.
- [X] T062 [US4] In `frontend/src/components/ai/HITLApprovalCard.tsx`, when `family_gap_mentions` is not empty, show the warning from `contracts/ui-copy.md` plus the snippets, and keep «Aprobar» and «Rechazar» available. Add tests for the warning shown and hidden, and jest-axe.
- [X] T063 [US4] In `frontend/src/components/newsletter/studio/BlockPanel.tsx`, add a one-click «Insertar aviso de cambios» that pre-fills `coach_note` with the notice from `contracts/ui-copy.md` (`CHANGE_NOTICE_TEXT` in `changeNotice.ts`: the condensed 59-word, 367-character text, because the backend caps `coach_note` at 60 words and 600 characters; the first 74-word draft could not be saved). The coach can edit it. Remember a dismissal per coach in the `localStorage` key `tyr:045-notice-dismissed:v1:{userId}`, and never send anything automatically. Add tests.

---

## Phase 8: Polish, audits and closing

- [X] T064 [P] Update the e2e specs in `frontend/e2e/`:
...
- [X] T065 Run the `data-privacy-guard` agent (Sonnet) over the whole 045 diff: parent payload omissions, no PII in logs or fixtures, `athlete_ref` never logged, `family_gap_mentions` scoped to coaches. Fix its findings and record the verdict in `specs/045-competitions-one-place/qa.md`. — Done 2026-09-23: PASS-WITH-FINDINGS. F-1 (parent insight-detail `metrics_snapshot` carried winner/podium gaps) fixed with tests; F-2 (`/distribution` open to parents, contract says coach-only) and F-3 (realistic names in a pre-existing e2e fixture) are open for the owner. See `qa.md`.
- [X] T066 [P] Update `CLAUDE.md` § Cross-feature invariants (Race identity): commits are gated by pending candidates that involve that import's record keys, and imports can be `discarded`. Add a dated entry to `docs/technical-notes.md` and a 045 row to `docs/implementation-status.md`.
- [X] T067 [P] Write `docs/10-race-results/competitions-one-place.md` (English), covering the area map, the glossary, the metric definitions and the alias table. Link it from `docs/README.md` if that index lists feature docs.
- [X] T068 Deferred lane: run `pytest -m mysql` against `trocha_ruta_test` (the full lane, including the T024 downgrade), and record the result in `qa.md`.
- [X] T069 Deferred lane: `pytest -m golden` (SC-008), only if a race AI key is available. Otherwise record it as not run in `qa.md`. Open a follow-up in `docs/implementation-status.md` to regenerate the golden fixtures with the time-based percentile (research R-02).
- [X] T070 Deferred lane: run the Playwright specs from T064 on the isolated e2e stack (never on the dev or production DB), and record the result in `qa.md`.
...
- [X] T074 Mark season-summary analyses stale when a revised import changes results of their season: extend `backend/app/services/race/run_staleness.py::invalidate_runs_for_event` (today it filters `AthleteAiInsight.event_id == event_id`, and season summaries carry `event_id=NULL`) or add a season-scoped variant called from the same commit path in `routers/race_imports.py`, with a test that a committed revision surfaces the summary in `pending-analyses?state=stale` per spec Key Entities «AI analysis» lifecycle / FR-041 (missing)
- [X] T075 Make every progression point in `frontend/src/components/race/history/HistoryChart.tsx` navigate to its competition via `raceEventHref(audience, event_id)` (click and keyboard, ≥ 48 px effective target), with a test per role per FR-017 / US1/AC4 (missing)
- [X] T076 Link each championship card in `frontend/src/components/athletes/ai/ChampionshipReadingCard.tsx` to its competition via `raceEventHref(audience, event_id)` (pass `event_id` from `ProgressionView.tsx`), with a test per role per FR-017 (missing)
- [X] T077 Replace the internal pipeline-stage labels in the expanded per-node list of `frontend/src/components/ai/AnalysisRunTimeline.tsx` (`GRAPH_NODES`: «Anonimizar datos», «Rehidratar nombres», «Agente crítico», «Revisión humana (HITL)»…) with the plain-language phase vocabulary already used in `PHASE_BY_NODE` per FR-004 (partial)
- [X] T078 Add `min-h-12` to the «Cancelar» and «Guardar y aprobar» buttons of the edit dialog footer in `frontend/src/components/ai/HITLApprovalCard.tsx` and assert it in the component test per FR-063 / SC-010 (partial)
- [X] T079 Render «sin dato» instead of «—» for missing position, parrilla and percentile in `frontend/src/components/athletes/ai/ChampionshipReadingCard.tsx` (reuse the shared formatters in `raceHistoryFormat.ts`) per US2/AC3 / FR-020 (contradicts)
- [X] T080 Render «sin dato» instead of «—» in the four empty branches of `backend/templates/documents/pdf/charts/championship_card.html.jinja` (family PDF) and cover it in `tests/services/notification/test_stage_log_pdf.py` per US2/AC3 / FR-020 (contradicts)
- [X] T081 Count unlinked competitors needing action in the «Cargas e identidades» menu badge (add the count to `CoachSummaryOut` and `frontend/src/hooks/layout/useNavBadges.ts`), or record the owner's acceptance of the two-kind badge from `research.md` in `spec.md` FR-033 per FR-033 (partial)
- [X] T082 Regenerate the metrics in `backend/evals/race_analyst/golden_v3/case_*.json` with `compute_field_metrics`/`compute_category_metrics` (they still carry position-based percentiles), re-run `pytest -m golden` and record the new baseline per SC-008 / research R-02 (partial)
- [X] T083 Branch the delete dialog copy in `frontend/src/routes/competitions/CompetitionDetailPage.tsx` on `event.is_championship` («Eliminar campeonato» vs «Eliminar válida») per FR-002 (partial)
- [X] T084 Rename the Home «Pendientes» row label «Insights IA desactualizados» in `frontend/src/components/dashboard/PendingInbox.tsx` to the «Análisis IA» vocabulary (for example «Análisis desactualizados») and update its tests per FR-002 / FR-010 (partial)
