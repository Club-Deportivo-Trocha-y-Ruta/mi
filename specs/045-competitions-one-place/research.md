...
- **Menu badge**: `CoachSummaryOut` gains `identity_decisions_pending` and `imports_in_progress`. `useNavBadges` shows their sum on «Cargas e identidades», so there are no three extra requests.
