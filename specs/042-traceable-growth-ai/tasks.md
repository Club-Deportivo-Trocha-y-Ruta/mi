...
- [ ] T047 [US5] Create `backend/evals/anthropometry_analyst/baseline.json` with the shipped prompt/model baseline scores and the 0.75 composite threshold. **BLOCKED in this environment**: the file exists with `status: PLACEHOLDER` scores and an embedded `regenerate_command`; real baseline numbers require a model API key, which this container does not have. Mirrors the accepted pattern of `backend/evals/race_analyst/baseline_2026-05-20.json`.
