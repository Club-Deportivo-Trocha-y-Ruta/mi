/**
 * IdentitySection — «¿Es la misma persona?»: revisión de identidad de
 * competidores dentro de «Cargas e identidades» (feature 045, US3; nace del
 * cuerpo de `IdentityReviewPage`, feature 044 US4).
 *
 ...
 *
 * Con `importId` (llegó desde el bloqueo de una carga, `?import=<id>`) muestra
 * un aviso con «Volver a la carga»: la carga quedó guardada y se retoma en el
 * asistente sin volver a subir el archivo.
 */
...
import { useRaceImport } from "@/hooks/ai/useRaceImports";
import {
...
export interface IdentitySectionProps {
  /** Id de la carga desde la que se llegó (`?import=<id>`), si aplica. */
  importId?: string | null;
}

/** Aviso «estás resolviendo decisiones de esta carga» + regreso al asistente. */
function ResumeImportBanner({ importId }: { importId: string }) {
  const importQuery = useRaceImport(importId);
  const detail = importQuery.data;
  // Si la carga ya no se puede retomar (confirmada, descartada, otro club),
  // no se ofrece un regreso que llevaría a un aviso vacío.
  const canResume =
    detail != null && (detail.status === "pending" || detail.status === "dry_run");
  if (!canResume) return null;
  return (
    <Card
      className="border border-amber-200 bg-amber-50"
      data-testid="identity-resume-banner"
    >
      <CardContent className="flex flex-wrap items-center justify-between gap-3 p-4">
        <p className="text-sm text-amber-900">
          Estás resolviendo decisiones de la carga
          {detail.source_filename ? ` «${detail.source_filename}»` : ""}. Tu
          carga quedó guardada: al terminar, vuelve y confírmala.
        </p>
        <Link
          to={`/competitions/import?import=${importId}`}
          className="inline-flex min-h-12 items-center gap-1.5 rounded-lg bg-charcoal px-3 py-2 text-sm font-semibold text-surface hover:opacity-90"
          data-testid="identity-back-to-import"
        >
...
export function IdentitySection({ importId = null }: IdentitySectionProps) {
  ...
  return (
    <div className="space-y-5" data-testid="identity-section">
      {importId && <ResumeImportBanner importId={importId} />}

      <div className="flex flex-wrap items-start justify-between gap-3">
        <p className="min-w-0 flex-1 text-sm text-mid-gray">
