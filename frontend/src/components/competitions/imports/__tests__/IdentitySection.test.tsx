/**
 * Tests de IdentitySection — «¿Es la misma persona?» de «Cargas e
 * identidades» (feature 045, US3; antes `IdentityReviewPage`, feature 044,
 * US4, T053).
 *
 * Cubre: pendientes, vacío, error, resume (progreso refleja lo ya decidido),
 * deshacer, 409 al decidir, atajo de teclado, y el aviso «Volver a la carga»
 * cuando se llega desde el bloqueo de una carga (`?import=<id>`); cero
 * violaciones axe en la sección y en el diálogo de confirmación.
 *
...
import { IdentitySection } from "@/components/competitions/imports/IdentitySection";
import {
  makeImportDetail,
...
function renderPage(importId: string | null = null) {
...
describe("IdentitySection", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mswServer.use(...raceIdentityHandlers, ...raceImportsHistoryHandlers);
  });
  ...
  it("sin ?import: no muestra el aviso «Volver a la carga»", async () => {
    renderPage();
    await screen.findByTestId("candidate-left");
    expect(
      screen.queryByTestId("identity-resume-banner"),
    ).not.toBeInTheDocument();
  });

  it("con ?import=<id> de una carga en curso: ofrece «Volver a la carga» al asistente con ?import=<id>", async () => {
    renderPage("7");
    const banner = await screen.findByTestId("identity-resume-banner");
    expect(banner).toHaveTextContent("Tu carga quedó guardada");
    expect(banner).toHaveTextContent("«valida_1_2026.pdf»");
    expect(screen.getByTestId("identity-back-to-import")).toHaveAttribute(
      "href",
      "/competitions/import?import=7",
    );
  });

  it("con ?import=<id> de una carga que ya no se puede retomar: no ofrece el regreso", async () => {
    let detailRequested = false;
    mswServer.use(
      http.get("*/api/race-analysis/imports/:id", ({ params }) => {
        detailRequested = true;
        return HttpResponse.json(
          makeImportDetail({ id: Number(params.id), status: "committed" }),
        );
      }),
    );
    renderPage("7");
    await screen.findByTestId("candidate-left");
    // La consulta del detalle SÍ ocurrió (no es una ausencia vacía)…
    await waitFor(() => expect(detailRequested).toBe(true));
    await new Promise((resolve) => setTimeout(resolve, 50));
    // …y como la carga ya se confirmó, no hay a dónde volver.
    expect(
      screen.queryByTestId("identity-resume-banner"),
    ).not.toBeInTheDocument();
  });

  it("sin violaciones de accesibilidad con el aviso de regreso visible", async () => {
    const { container } = renderPage("7");
    await screen.findByTestId("identity-resume-banner");
    const results = await axe(container);
    expect(results).toHaveNoViolations();
  });
});
