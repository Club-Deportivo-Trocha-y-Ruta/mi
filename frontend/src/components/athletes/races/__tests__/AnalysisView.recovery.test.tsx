/**
 * Recuperación de runs activos al montar la vista «Análisis IA» de Carreras.
 *
 * Feature 045 (T042): re-alojada desde `AthleteAIAnalysisTab.recovery.test.tsx`
 * — la lógica de recuperación (run vivo, HITL) se movió tal cual a
 * `races/AnalysisView.tsx` cuando se retiró `AthleteAIAnalysisTab`; `mode`
 * pasó a `audience` ("parent" → "family"). El bloque "pendiente" ahora es
 * visible sin cambiar de sub-tab (ya no existen los sub-tabs).
 *
 ...
 *
 * La sesión mockeada es SIEMPRE de coach: así el caso `audience="family"`
 * prueba el gate por audiencia del componente y no el gate por rol que
...
describe("AnalysisView — recuperación de runs activos", () => {
  ...
  it("audience=family NO pide el listado de runs (endpoint coach-only)", async () => {
    useRunsHandler(RUNS_PAGE);
    mswServer.use(hitlWaitingRunStatusHandler());
