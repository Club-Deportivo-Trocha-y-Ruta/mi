...
| 04 | [04-percentiles/](04-percentiles/) | WHO/CDC percentiles: research + original implementation of growth curves. `workflow.md` superseded 2026-09-04 by the growth module redesign — see row 18 / `specs/040-growth-module-redesign` |
...
| 10 | [10-race-results/](10-race-results/) | Copa Valle XCO results: PDF ingestion, fuzzy normalization, longitudinal analytics (progression, podium gap, club ranking, projection). Extension 2026-05-26: race conditions in UI (wizard + tri-state card + PATCH) — see `upload-design.md` §14. Extension 2026-05-27: **Competitions** module (CRUD `race_events`, relocated wizard, URL-driven tabs) — see `competitions-module.md`. Extension 2026-09-02: **AI Insights v3** (specs/037), causal/field-relative/prescriptive structured analysis replacing the v2 3-section markdown contract — see `spec-insights-v3.md`. Extension 2026-09-16: **Course profile** (specs/043), optional per-válida GPX-derived circuit with derived distance/speed — see `course-profile-design.md`. Extension 2026-09-22: **Race history backfill** (specs/044, on branch, not deployed), Copa Valle 2024–2025 with cross-season athlete progression — see `history-backfill-design.md` and `runbook-ops.md` §12 |
...
| 18 | [18-growth-module-redesign/](18-growth-module-redesign/) | **Growth module redesign** (input for specs/040): audit of the `Crecimiento` tab (CDC/WHO double standard, parent latest-record bug, layout, design-system drift) + phased work plan — `proposal.md`. Implemented 2026-09-04 as `specs/040-growth-module-redesign` (WHO 2007 single reference, `growth-summary` endpoint, decision-first tab, family view) — status in `docs/implementation-status.md`'s own entry |
| 19 | [19-multi-coach-governance/](19-multi-coach-governance/) | **Multi-coach governance** (specs/041): actor+timestamp on every write, athlete archive/restore, sessions led by several coaches, club audit trail with 24-month purge — `design.md` + `qa.md` + `runbook.md` + `policy-v1.3-draft.md` (unpublished privacy-policy draft) |
| 20 | [20-traceable-growth-ai/](20-traceable-growth-ai/) | **Traceable growth AI** (specs/042): anthropometry explainer rebuilt as a plain async pipeline (context → analyst → critic → guardrails → persist) on the shared LangChain layer, redact-always Langfuse tracing, golden eval — `design.md` + `qa.md` + `runbook.md` |
| 21 | [21-body-composition/](21-body-composition/) | **Body composition (skinfolds) research** (input for a future spec, 2026-09-23): protocol for ages 9–15 with a plastic caliper (`research-protocol.md`), safeguards + traffic-light bands + referral pathway (`research-safeguards-referral.md`), illustrated capture UX (`research-measurer-ux.md`), technical fit on the anthropometry module (`technical-fit.md`), synthesis and owner decisions (`proposal.md`). Specified 2026-09-23 as `specs/046-body-composition-skinfolds` (on `main`, no branch); not implemented |
