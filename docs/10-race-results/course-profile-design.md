...
| Parents see course data only for válidas their own children are related to, and `my_categories` only their own athletes. | `_get_course_for_parent` gates visibility on results of the caller's `allowed_athlete_ids` and derives `my_categories` from that same `RaceResult` set; an unrelated parent gets `404 course_not_available`, not a filtered `200`. |
