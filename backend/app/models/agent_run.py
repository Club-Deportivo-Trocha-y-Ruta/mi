...
Nota: ``langfuse_trace_id`` solo se llena en desarrollo local con
``LANGFUSE_ENABLED=true`` (``services/race/observability.py``, escrito en
``routers/race_analysis.py::_finalize_run``); en producción queda ``NULL``.
Langfuse es un observador de tokens por llamada, no fuente de verdad de
costos (esa sigue siendo ``agents/pricing.py`` + budget guard).
"""
