...
- **Two separate AI stacks** — do not conflate their config:
  - `app/services/ai/` — provider factory (`anthropic` default / `openai` / `google` / `fake`) on `AI_*` env vars. Powers the session assistant, monthly reports, and newsletters. `AI_ENABLED=false` yields `FakeLLMProvider`. The Anthropic provider must NOT forward `temperature` (Claude 4.6+ returns 400 on non-default sampling params).
  - `app/services/race/agents/` — LangGraph agentic pipeline (analyst → critic → chat) with its own `RACE_AI_*` provider config, Langfuse observability, and sqlite checkpointing. Guarded by a golden eval (`backend/evals/race_analyst/`), blocking in CI at composite ≥ 0.75 (`.github/workflows/race-eval.yml`).
