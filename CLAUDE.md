...
| Structured Interval Training with Strava Correlation (coach-authored interval structures — warmup/repeatable work-recovery groups/cooldown, HR zone + cadence targets, **no power column** — attached 1:1 to a training session; brand-specific PDF instructivo iGPSport/Magene/Garmin; automatic plan-vs-actual comparison via new persisted `strava_activity_laps` — never GPS — matched sequentially against flattened blocks with ±30% tolerance, deferred via `TaskDispatcher` on link/structure-change + manual recalculate; club-scoped tagged template library with copy-on-attach; hybrid age gating — hard block Z3+ on 10-12, confirm-and-record otherwise; coach/admin-only, parents/athletes 403) | specs/026-structured-interval-training | ✅ Complete — deploy pending (migration b5c6d7e8f9a0; run migración en Render) |
