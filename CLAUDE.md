...
**Open after 042**: `backend/evals/anthropometry_analyst/baseline.json` holds `PLACEHOLDER` scores until someone regenerates it with a real model key; `.github/workflows/anthropometry-eval.yml` needs a new `AI_API_KEY` repo secret (distinct from `RACE_AI_API_KEY`) before it can run; the `-m mysql` column test, `frontend/e2e/growth-analysis.spec.ts` and the golden eval are written but unexecuted (they need the local stack); SC-005's moderated check is unrecorded. Feature 041 (multi-coach governance) is verified locally; feature 040's deferred items (T027/T028 MySQL `_test` recompute, Playwright specs, T077–T079) remain open and are now written against the traced path.
<!-- SPECKIT END -->
