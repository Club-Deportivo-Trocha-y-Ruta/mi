...
Two hard rules color everything:
...
- **Two separate AI stacks** — do not conflate their config:
  ...
  - `app/services/race/agents/` — LangGraph agentic pipeline (analyst → critic → chat) with its own `RACE_AI_*` provider config, default `google`/`gemini-3.1-flash-lite` (the coach relies on Gemini's free quota; `anthropic`/`claude-sonnet-5` and `openai` are supported alternatives, not the default), and sqlite checkpointing (`./data/langgraph_state.sqlite`, ephemeral on Render's free-tier filesystem — a pending HITL decision does not survive a deploy; orphan runs are reconciled at startup). No observability tool is wired in — the audit trail is `athlete_ai_insights` / `agent_runs` / `agent_run_events`; self-hosted Langfuse was scoped as an optional future add-on and was never implemented. Guarded by a golden eval (`backend/evals/race_analyst/`), blocking in CI at composite ≥ 0.75 (`.github/workflows/race-eval.yml`).
...
<!-- SPECKIT START -->
Active feature plan: `specs/036-ai-insights-tab-review/plan.md` (spec, research, data-model, contracts and quickstart live alongside it).
<!-- SPECKIT END -->
